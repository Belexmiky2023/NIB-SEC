...
const SetupView: React.FC<SetupViewProps> = ({ initialData, onComplete }) => {
  ...
  // Expanded taken list
  const takenUsernames = ['@nibsec', '@nibsecoffical', '@nibsecurity', '@oryn', '@oryn179', '@179oryn', '@admin', '@root', '@bee'];
    ...
    if (cleanUsername.length > 1) {
      ...
      const timer = setTimeout(() => {
        ...
        if (takenUsernames.includes(cleanUsername)) {
          ...
          setError('Username can not be number.');
          setSuggestions([
            `@user_${namePart}`,
            `@bee_${namePart}`,
            `@nib${namePart}`
    ...
    <div className="h-full flex items-center justify-center p-6 bg-black/40 backdrop-blur-xl">
      <div className="max-w-md w-full bg-[#0a0a0a] border border-yellow-400/20 rounded-[48px] p-10 space-y-8 relative overflow-hidden shadow-[0_0_100px_rgba(250,204,21,0.1)]">
        ...
        <div className="text-center space-y-2">
          <h2 className="text-4xl font-black text-yellow-400 uppercase tracking-tighter italic">Profile Setup</h2>
          <p className="text-gray-500 text-[10px] uppercase font-bold tracking-widest">Initialization Protocol 1.0</p>
        </div>
        ...
        <div className="flex flex-col items-center space-y-4">
          <div className="relative group">
            ...
            <label className="absolute bottom-1 right-1 bg-yellow-400 text-black w-10 h-10 rounded-full flex items-center justify-center cursor-pointer hover:bg-white transition-all border-4 border-[#0a0a0a] shadow-xl">
        ...
        <div className="space-y-5">
          <div className="space-y-2">
            <label className="text-[10px] uppercase tracking-[0.3em] text-gray-600 px-4 font-black">Operative Name</label>
            <input 
              type="text" 
              value={displayName}
          ...
          <button 
            ...
            className={`w-full py-5 rounded-[2rem] font-black uppercase tracking-[0.4em] text-xs transition-all ${
              isValid 
                ? 'bg-yellow-400 text-black hover:scale-105 shadow-[0_15px_40px_rgba(250,204,21,0.3)] active:scale-95' 
