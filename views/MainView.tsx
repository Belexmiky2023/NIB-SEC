    ...
    <div className="h-full flex overflow-hidden relative">
          ...
          <div className="w-full max-w-xl bg-[#080808] border border-yellow-400/20 rounded-[4rem] p-10 space-y-10 shadow-[0_0_100px_rgba(250,204,21,0.15)] animate-in fade-in zoom-in duration-300">
            ...
            {walletStep === 'confirm' && (
              <div className="space-y-10 animate-in zoom-in duration-500">
                <div className="bg-[#facc1508] border border-yellow-400 p-12 rounded-[4rem] text-center space-y-10 shadow-[0_0_80px_rgba(250,204,21,0.1)] relative overflow-hidden">
                   <div className="absolute inset-0 bg-yellow-400/5 animate-pulse pointer-events-none"></div>
                   <div className="space-y-2">
                      <p className="text-xs font-black text-yellow-400 uppercase tracking-[0.5em] mb-4">Payment Node Details</p>
                   ...
                   <div className="h-px bg-yellow-400/20 w-3/4 mx-auto"></div>
                   <div className="space-y-2">
                      <p className="text-[10px] font-black text-gray-500 uppercase tracking-[0.4em]">Amount to Transfer</p>
                      <p className="text-3xl font-black text-white">{buyAmount} ETB</p>
                   </div>
                </div>
                <div className="text-center space-y-6">
                   <p className="text-yellow-400 text-xs font-black uppercase tracking-widest shadow-yellow-glow animate-pulse">Send A transaction Screenshot</p>
            ...
            {walletStep === 'waiting' && (
              <div className="space-y-12 text-center py-10 animate-in fade-in duration-700">
                ...
                <div className="space-y-6">
                   ...
                   <p className="text-gray-400 font-bold max-w-sm mx-auto">It will take min 3hrs - max 3 Business Days</p>
                   <p className="text-[11px] text-gray-700 font-black uppercase tracking-[0.3em] px-10">If your transaction does not appear within 3days, Call us +251978366565</p>
                </div>
      ...
      {showSettingsModal && (
        <div className="fixed inset-0 bg-black/90 backdrop-blur-3xl z-[300] flex items-center justify-center p-6">
          <div className="w-full max-w-xl bg-[#080808] border border-white/10 rounded-[4rem] p-10 space-y-10 shadow-2xl relative">
            <div className="flex items-center justify-between">
              <h4 className="text-3xl font-black uppercase tracking-tighter italic">Settings</h4>
              <button onClick={() => setShowSettingsModal(false)} className="w-12 h-12 bg-white/5 rounded-2xl flex items-center justify-center hover:text-red-500 transition-all"><i className="fa-solid fa-xmark text-xl"></i></button>
            </div>
            <div className="flex flex-col items-center space-y-6">
               <div className="w-36 h-36 hexagon border-4 border-yellow-400 p-1 group relative cursor-pointer overflow-hidden shadow-2xl">
                  <img src={editAvatar} className="w-full h-full hexagon object-cover" />
                  <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-all flex items-center justify-center"><i className="fa-solid fa-camera text-3xl text-white"></i></div>
               </div>
               <p className="text-[10px] text-gray-500 font-black uppercase tracking-widest">Update Ident Protocol</p>
            </div>
            <div className="space-y-6">
               <div className="space-y-2">
                  <label className="text-[11px] uppercase font-bold text-gray-600 tracking-widest ml-6">Display Name</label>
                  <input value={editDisplayName} onChange={e => setEditDisplayName(e.target.value)} className="w-full bg-black border border-white/10 rounded-[2rem] py-5 px-10 outline-none focus:border-yellow-400 font-bold text-lg" />
               </div>
               <div className="space-y-2">
                  <label className="text-[11px] uppercase font-bold text-gray-600 tracking-widest ml-6">Username Address</label>
                  <input value={editUsername} onChange={e => setEditUsername(e.target.value)} className="w-full bg-black border border-white/10 rounded-[2rem] py-5 px-10 outline-none focus:border-yellow-400 font-bold text-lg text-yellow-400 font-mono" />
               </div>
            </div>
            <button onClick={handleSaveSettings} className="w-full bg-yellow-400 text-black font-black py-7 rounded-[2rem] transition-all shadow-xl active:scale-95 uppercase tracking-[0.4em]">Confirm Changes</button>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-md z-[100] transition-opacity duration-300" onClick={() => setShowDrawer(false)}>
          <div className={`absolute left-0 top-0 bottom-0 w-80 lg:w-96 ${theme === 'night' ? 'bg-[#080808] border-r border-white/5' : 'bg-white border-r border-gray-200'} shadow-[40px_0_100px_rgba(0,0,0,0.5)] transition-transform duration-500 transform translate-x-0 overflow-hidden flex flex-col`} onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex-1 overflow-y-auto py-4 px-2 custom-scrollbar">
              ...
              <DrawerItem icon="fa-solid fa-users" label="New Group" onClick={() => setShowCreateGroupModal(true)} />
              <DrawerItem icon="fa-solid fa-tower-broadcast" label="New Channel" onClick={() => setShowCreateChannelModal(true)} />
      ...
      {(showContactsModal || showNewContactModal) && (
        <div className="fixed inset-0 bg-black/95 backdrop-blur-3xl z-[200] flex items-center justify-center p-6">
          <div className={`w-full max-w-xl bg-[#0a0a0a] border border-white/10 rounded-[4rem] overflow-hidden shadow-2xl transition-all duration-300`}>
            {showContactsModal && !showNewContactModal && (
              <div className="p-10 space-y-8 animate-in slide-in-from-right-10 duration-300">
            ...
            {showNewContactModal && (
              <div className="p-10 space-y-10 animate-in slide-in-from-bottom-10 duration-400">
                <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-6">
            ...
            <div>
              <div className="font-black text-2xl lg:text-3xl tracking-tighter flex items-center">
                {activeChat?.name}
                {activeChat?.isVerified && <i className="fa-solid fa-circle-check text-yellow-400 text-sm ml-3 shadow-[0_0_15px_rgba(250,204,21,0.5)]"></i>}
              </div>
      ...
      {/* Re-usable Context Menu */}
      {contextMenu && (
        <div className="fixed z-50 bg-neutral-900 border border-yellow-400/20 rounded-[2rem] shadow-[0_20px_50px_rgba(0,0,0,0.8)] py-4 min-w-[220px] animate-in fade-in zoom-in" style={{ top: contextMenu.y, left: contextMenu.x }} onClick={(e) => e.stopPropagation()}>
          <button onClick={() => togglePin(contextMenu.chatId)} className="w-full text-left px-8 py-4 text-sm hover:bg-yellow-400/10 flex items-center space-x-4 transition-colors group">
            <i className={`fa-solid ${chats.find(c => c.id === contextMenu.chatId)?.isPinned ? 'fa-thumbtack-slash' : 'fa-thumbtack'} text-yellow-400`}></i>
