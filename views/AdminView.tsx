...
const AdminView: React.FC<AdminViewProps> = ({ onExit }) => {
  ...
  return (
    <div className="h-full flex flex-col bg-black text-white font-mono animate-in fade-in duration-700">
      ...
      <div className="flex flex-1 overflow-hidden">
        <div className="w-72 bg-[#050505] border-r border-white/5 p-6 space-y-4">
          ...
          <button onClick={() => setActiveTab('vault')} className={`w-full flex items-center space-x-4 p-5 rounded-2xl transition-all ${activeTab === 'vault' ? 'bg-yellow-400 text-black font-black' : 'hover:bg-white/5 text-gray-500'}`}>
            <i className="fa-solid fa-vault"></i><span className="uppercase tracking-widest text-xs">Vault Requests</span>
          ...
          <button onClick={() => setActiveTab('network')} className={`w-full flex items-center space-x-4 p-5 rounded-2xl transition-all ${activeTab === 'network' ? 'bg-yellow-400 text-black font-black' : 'hover:bg-white/5 text-gray-500'}`}>
            <i className="fa-solid fa-network-wired"></i><span className="uppercase tracking-widest text-xs">Network Controls</span>
          </button>
        </div>
        ...
        <div className="flex-1 p-10 overflow-y-auto custom-scrollbar bg-black">
          {activeTab === 'operatives' && (
            <div className="space-y-8">
               <div className="flex items-center justify-between mb-10">
                  ...
                  <div className="bg-white/5 border border-white/10 px-6 py-3 rounded-2xl text-xs text-gray-500 font-black">Live Nodes: {liveNodes}</div>
               </div>
                 ...
                 {operatives.map(op => (
                   <div key={op.id} className={`p-8 rounded-[3rem] border transition-all ${op.isBanned ? 'bg-red-500/5 border-red-500/20' : 'bg-neutral-900/50 border-white/5 hover:border-yellow-400/20 shadow-2xl'}`}>
                      <div className="flex items-center justify-between mb-8">
                        <div className="flex items-center space-x-6">
                           <div className="w-16 h-16 hexagon p-1 bg-yellow-400/20 relative">
                              ...
                              <div className="absolute -top-2 -left-2 bg-black border border-white/10 rounded-full p-1.5 flex items-center justify-center">
                           ...
                           <div>
                              <div className="flex items-center space-x-2">
                                <p className="text-xl font-black">{op.displayName}</p>
                      ...
                      <div className="flex items-center space-x-3">
                         <button onClick={() => handleAction(op.id, 'verify')} className={`flex-1 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest ${op.isVerified ? 'bg-yellow-400 text-black' : 'bg-white/5 text-gray-500'}`}>{op.isVerified ? 'Unverify' : 'Verify'}</button>
                         <button onClick={() => handleAction(op.id, 'add_nib')} className="flex-1 py-3 bg-white text-black rounded-2xl text-[10px] font-black uppercase tracking-widest">Add 100 NIB</button>
                         <button onClick={() => handleAction(op.id, 'ban')} className={`flex-1 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest ${op.isBanned ? 'bg-white text-black' : 'bg-red-600 text-white'}`}>{op.isBanned ? 'Revoke' : 'Ban'}</button>
                      </div>
                   </div>
                 ))}
               </div>
          ...
          {activeTab === 'vault' && (
             <div className="space-y-8 text-center animate-in slide-in-from-right-10">
               ...
               {payments.length === 0 ? <p className="text-gray-600">No pending transfers.</p> : (
                 <div className="space-y-6">
                   {payments.map(pay => (
                     <div key={pay.id} className="bg-[#0a0a0a] border border-yellow-400/10 p-10 rounded-[4rem] flex items-center justify-between">
                        <div className="flex items-center space-x-10"><i className="fa-solid fa-building-columns text-blue-500 text-3xl"></i><div><p className="text-2xl font-black">{pay.username}</p><p className="text-xs text-gray-600">{pay.method}</p></div></div>
                        <div className="flex items-center space-x-12"><p className="text-3xl font-black text-yellow-400">{pay.amount} ETB</p><button onClick={() => approvePayment(pay.id)} className="h-16 px-10 bg-yellow-400 text-black font-black rounded-3xl">Approve</button></div>
                     </div>
                   ))}
                 </div>
               )}
          ...
          {activeTab === 'network' && (
             <div className="h-full flex flex-col items-center justify-center space-y-10 animate-in zoom-in text-center">
                <i className="fa-solid fa-tower-broadcast text-yellow-400 text-8xl animate-pulse"></i>
                <h3 className="text-5xl font-black italic uppercase tracking-tighter">Mainframe Online</h3>
                <div className="grid grid-cols-2 gap-6 w-full max-w-2xl">
                   <div className="bg-[#0a0a0a] border border-white/5 p-8 rounded-[3rem]"><p className="text-[10px] text-gray-600">Global Encryption</p><p className="text-2xl font-black text-green-500">ACTIVE</p></div>
                   <div className="bg-[#0a0a0a] border border-white/5 p-8 rounded-[3rem]"><p className="text-[10px] text-gray-600">Active Nodes</p><p className="text-2xl font-black text-yellow-400">{liveNodes}</p></div>
                </div>
             </div>
          )}
        </div>
