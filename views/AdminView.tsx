...
const mockOperatives: User[] = [
  { id: '1', username: '@nibsecoffical', displayName: 'NIB HQ', avatarUrl: 'https://i.ibb.co/3ykXF4K/nib-logo.png', isProfileComplete: true, walletBalance: '1000000', isVerified: true },
  { id: '2', username: '@oryn', displayName: 'Oryn', avatarUrl: 'https://picsum.photos/200', isProfileComplete: true, walletBalance: '500', isVerified: true },
  { id: '3', username: '@bee_user', displayName: 'Busy Bee', avatarUrl: 'https://picsum.photos/201', isProfileComplete: true, walletBalance: '50' },
  { id: '4', username: '@hacker_01', displayName: 'Unknown Node', avatarUrl: 'https://picsum.photos/202', isProfileComplete: true, walletBalance: '0', isBanned: true },
];
    ...
    <div className="h-full flex flex-col bg-black text-white font-mono">
      ...
      <div className="h-24 bg-[#0a0a0a] border-b border-yellow-400/20 px-10 flex items-center justify-between shadow-[0_0_50px_rgba(250,204,21,0.1)]">
        <div className="flex items-center space-x-6">
          <div className="w-12 h-12 bg-yellow-400 text-black rounded-xl flex items-center justify-center text-2xl shadow-yellow-glow">
            <i className="fa-solid fa-user-shield"></i>
          </div>
          <div>
            <h2 className="text-2xl font-black uppercase italic tracking-tighter">The Hive Overseer</h2>
            <p className="text-[10px] text-yellow-400/60 font-black uppercase tracking-[0.4em]">Master Control Interface v1.0</p>
          </div>
        </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 p-10 overflow-y-auto custom-scrollbar bg-black">
                 ...
                 {operatives.map(op => (
                   <div key={op.id} className={`p-8 rounded-[3rem] border transition-all ${op.isBanned ? 'bg-red-500/5 border-red-500/20' : 'bg-neutral-900/50 border-white/5 hover:border-yellow-400/20 shadow-2xl'}`}>
                      <div className="flex items-center justify-between mb-8">
                        <div className="flex items-center space-x-6">
                           <div className="w-16 h-16 hexagon p-1 bg-yellow-400/20">
                      ...
                      <div className="flex items-center space-x-3">
                         <button 
                           onClick={() => handleAction(op.id, 'verify')}
                           className={`flex-1 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${op.isVerified ? 'bg-yellow-400 text-black' : 'bg-white/5 text-gray-500 border border-white/5'}`}
                         >
                            {op.isVerified ? 'Unverify' : 'Verify'}
                         </button>
                         <button 
                           onClick={() => handleAction(op.id, 'add_nib')}
                           className="flex-1 py-3 bg-white text-black rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-yellow-400 transition-all"
                         >
                            Add 100 NIB
                         </button>
                         <button 
                           onClick={() => handleAction(op.id, 'ban')}
                           className={`flex-1 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${op.isBanned ? 'bg-white text-black' : 'bg-red-600 text-white shadow-lg shadow-red-600/20'}`}
                         >
                            {op.isBanned ? 'Revoke Ban' : 'Terminate'}
                         </button>
                      </div>
                   </div>
                 ))}
          ...
          {activeTab === 'vault' && (
             <div className="space-y-8 animate-in slide-in-from-right-10 duration-500">
               <div className="flex items-center justify-between mb-10">
                  ...
                  <div className="bg-red-500/10 border border-red-500/20 px-6 py-3 rounded-2xl text-xs text-red-500 font-black">Pending Protocols: {payments.length}</div>
               </div>
               ...
               {payments.length === 0 ? (
                 ...
                 <div className="space-y-6">
                   {payments.map(pay => (
                     <div key={pay.id} className="bg-[#0a0a0a] border border-yellow-400/10 p-10 rounded-[4rem] flex items-center justify-between shadow-2xl relative overflow-hidden group">
                        ...
                        <div className="flex items-center space-x-12 relative z-10">
                           <div className="text-center">
                              ...
                              <p className="text-[10px] text-gray-700 font-black uppercase tracking-widest">TRANSIT LOAD</p>
                           </div>
                           <div className="flex items-center space-x-4">
                              <button 
                                onClick={() => setPayments(prev => prev.filter(p => p.id !== pay.id))}
                                className="w-16 h-16 bg-neutral-900 hover:bg-red-600 border border-white/10 rounded-3xl flex items-center justify-center text-gray-600 hover:text-white transition-all"
                              >
                                <i className="fa-solid fa-xmark"></i>
                              </button>
                              <button 
                                onClick={() => approvePayment(pay.id)}
                                className="h-16 px-10 bg-yellow-400 text-black font-black uppercase tracking-[0.3em] rounded-3xl hover:bg-white shadow-xl transition-all"
                              >
                                Approve Signal
                              </button>
                           </div>
          ...
          {activeTab === 'network' && (
             <div className="h-full flex flex-col items-center justify-center space-y-10 animate-in zoom-in duration-700">
                <div className="text-center space-y-6">
                   <div className="w-40 h-40 mx-auto text-yellow-400 text-8xl animate-pulse"><i className="fa-solid fa-tower-broadcast"></i></div>
                   <h3 className="text-5xl font-black italic uppercase italic tracking-tighter">Mainframe Control</h3>
                   <p className="text-gray-500 font-bold uppercase tracking-[0.4em]">Nodes globally online</p>
                </div>
                <div className="grid grid-cols-2 gap-6 max-w-2xl w-full">
                   <div className="bg-[#0a0a0a] border border-white/5 p-8 rounded-[3rem] text-center space-y-4">
